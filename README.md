...
The number of trees in our forest in `n_estimators` was changed as well as artificially limiting the depth of our trees with `max_depth` and the splits in our trees with `min_samples_split` to better capture generality rather than capture specfific patterns in our training data.
...
We believe our model has made significant improvement from milestone 3 as a result of fixing the problem we had with overfitting. Through combining our output classes and testing our model with extra hyperparameter tuning, we were able to produce an accuracy of 84% for both our training and testing set. From our graphs, we are observing sharp drops into plateaus which indicate further that our model is converging on the data correctly. Consistent loss once again reinforces the fact that from our training data we are no longer overfitting on the model and are accurately predicting without training bias.

Compared to the previous model, the  main differential is the difficulty in fitting. To reduce the overfitting problem from before required tuning the number of trees in our forest in `n_estimators` as well as artificially limiting the depth of our trees with `max_depth` to better capture generality rather than capture specfific patterns in our training data. Other hyperparameters were tested but were found to decrease overall accuracy.
...
Due to our implementation of the neural net being more from scratch, more time could have been spent on hyperparameters, but the lengthy time for training and the indication of likely low overfit was satisfying enough for this milestone. We could utilize sklearns gridsearchCV to make this training more automated. SVMs are definitely in consideration as we believe that support vectors will help us with our extremely variable features. Since we have experimented with hyperparameter tuning in our model, we also believe using SVMs will yield better class separation. Furthermore, class rebalancing to a higher degree can help us with the imbalance in false positives and false negatives generated by our model. While we have done some class rebalancing in our model already, it was mostly used to address our overfitting issue. We believe that further class rebalancing by implementing class-weighted loss function could create biases in the dataset that can improve our accuracy further.
